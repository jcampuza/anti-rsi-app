...
export const BreakOverlay: Component<BreakOverlayProps> = (props) => {
  ...
  const isActiveBreak = createMemo(() => breakState() === "in-mini" || breakState() === "in-work")
  const isWorkBreak = createMemo(() => breakState() === "in-work")
  const breakDuration = createMemo(() =>
    isWorkBreak() ? props.config.work.durationSeconds : props.config.mini.durationSeconds,
  )
  const elapsed = createMemo(() =>
    isWorkBreak() ? props.snapshot.timings.workTaking : props.snapshot.timings.miniTaking,
  )
  const remaining = createMemo(() => Math.max(breakDuration() - elapsed(), 0))
  const progressAnimationMs = createMemo(() => Math.max(props.config.tickIntervalMs + 160, 280))
          ...
          <Show when={props.onPostpone || props.onSkip}>
            <div class="flex items-center justify-center gap-3">
              <Show when={props.onPostpone}>
                <Button type="button" variant="primary" onClick={props.onPostpone}>
                  Postpone
                </Button>
              </Show>
