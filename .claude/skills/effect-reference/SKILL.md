...
2. Check whether effect, opencode, and t3code reference repos are already present in `.references/repos`
  - `https://github.com/Effect-TS/effect`: Effect v4 beta code (merged from the former `effect-smol` repository)
...
3. If the repositories are not present, clone them into .references/repos`, then explore there.
  ...
  ```
4. Read the workspace's installed Effect version, fetch tags in the Effect reference checkout, and check out the matching `effect@<version>` tag. Use the latest published beta tag when the task is an Effect beta upgrade.
